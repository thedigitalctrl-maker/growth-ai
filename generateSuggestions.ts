...
export function generateSuggestions(postText: string, tone: ToneType): CommentSuggestion[] {
  ...
  // For now, return mock suggestions based on tone
  return [
    {
      id: '1',
      text: examples[0],
...
export function generatePersonalizedSuggestions(
  ...
  const baseSuggestions = generateSuggestions(postText, tone);
  
  if (!profile) {
    return baseSuggestions;
  }
  
  // In production, this would use the profile to personalize
